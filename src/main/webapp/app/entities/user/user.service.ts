...
import { IAdminUser } from './user-management.model';

@Injectable({ providedIn: 'root' })
export class UserService {
  private resourceUrl = this.applicationConfigService.getEndpointFor('api/users');
  private resourceUrlForType = this.applicationConfigService.getEndpointFor('api/users/type');
  ...
  queryForType(type: string, req?: Pagination): Observable<HttpResponse<IAdminUser[]>>{
    const options = createRequestOption(req);
    return this.http.get<IAdminUser[]>(`${this.resourceUrlForType}/${type}`, { params: options, observe: 'response' });
  }
